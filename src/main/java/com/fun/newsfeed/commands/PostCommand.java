...
public class PostCommand implements UserCommand {
  ...
  public PostCommand(PostController postController) {
  ...
  public void execute() {
    ...
    try {
      long postId = postController.addUserPost(UserPostDto.newBuilder().withContentList(contentList).build());
      System.out.println(ColorConstants.ANSI_GREEN
          + "Successfully posted with postId: "+ postId + ColorConstants.ANSI_RESET);
